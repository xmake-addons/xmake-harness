...
有些活儿既不是一个问题，也不是一个周期，而是「必须变成真的」那么一件事。

```
/goal 让测试全过
/goal 5 web ui 在 linux 上零告警构建通过
```

它就是把周期任务的时钟拿掉：上一轮一结束，下一轮马上开始，直到目标达成。
第一轮提出这件事，之后每一轮都在问「到了没有」，并做下一件能推进它的事。

它自己会在三种情况下结束：agent 调用 `goal_done(reason)`（它验证过、目标确实达成了）、
轮次预算用完（默认 12 轮，也可以写在目标前面）、或者连续三轮失败。
`/goal stop` 和 `esc` 同样能结束它。

它和 `/loop` 的区别，是「盯着这个」和「把这个变成真的」的区别：
周期任务本来就没有终点，也不该有；而目标做完就是做完了，而且只有 agent 看得见它做完了。
除此之外两者是同一套机制 —— 每一轮都是同一个会话里的普通轮次，
同样的权限模式、同样的工具检查、同样吃缓存。
