...
**4. 硬上限兜底** —— 超过 `context.hardthreshold`（92%）后不再客气：所有工具输出
一律截断（包括最近的），还放不下就从最前面整轮丢弃，直到确定能装下。
请求超出窗口是服务端的硬错误，不是「答得差一点」，所以丢掉最老的几轮更划算。

为了让这个限制可信，估算器是**自校准**的：纯字符启发式只有 ±20% 的准头，
所以每次响应都把服务端返回的真实 prompt token 数按模型折进一个修正因子。
几轮之后误差收敛到几个百分点。
...
分级的意义是把大模型花在真正需要的地方。只读并汇报的子 agent —— explorer、
摘要这类 —— 用小模型就够，成本只有零头，所以它们**默认**就用小模型：
`tools` 里全是只读工具的 agent 自动跑在 `small` 上，除非定义里另行指定。
