...
```
 ...
 ─────────────────────────────────────────────────────────────────
  ...
  core/session                      the append-only event log
...
turn/start
  step
    context/compact if the window is nearly full
...
`compact` event. The transcript, the resume, the export and the token statistics are
all derived from the same log, so nothing can drift apart.
